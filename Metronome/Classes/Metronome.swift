...
//    }
    ...
    func scheduleBeats() {
        ...
        self.syncQueue.async {
            while self.beatsScheduled < self.beatsToScheduleAhead && self.isPlaying {
                self.setBeatValues()

                let sampleTime = self.playerBeatSampleValue
                let buffer = self.audioBuffers[self.bufferIndex]

                self.playerNode.scheduleBuffer(buffer, at: sampleTime) {
                    DispatchQueue.main.async {
                        self.beatsScheduled -= 1
                        self.scheduleBeatsIfNeeded() // Check if more beats need scheduling
                    }
                }
                ...
                let beatIndex = (self.beatNumber - 1) % self.beatsInBar
                let barIndex = (self.beatNumber - 1) / self.beatsInBar

                DispatchQueue.main.async {
                    self.onTick?(barIndex, beatIndex)
                    self.ticker.tick.toggle()
                }

                self.beatNumber += 1
                self.beatsScheduled += 1
                self.bufferIndex = (self.bufferIndex + 1) % self.beatsInBar
                self.nextBeatSampleValue += self.samplesPerBeat
            }
        }
    }

    func scheduleBeatsIfNeeded() {
        if self.beatsScheduled < self.beatsToScheduleAhead && self.isPlaying {
            scheduleBeats()
        }
    }
